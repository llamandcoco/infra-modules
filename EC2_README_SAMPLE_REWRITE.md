...
All examples are **tested configurations** located in the [`tests/`](tests/) directory. Each example includes a complete Terraform configuration you can copy and adapt for your use case.
...
```hcl
# Use configuration from tests as reference
module "web_server" {
  source = "github.com/llamandcoco/infra-modules//terraform/ec2?ref=v1.0.0"
...
- Private instance in existing VPC
- Default security best practices (IMDSv2, encryption)
- Basic monitoring and tagging
...
**📁 [View complete configuration →](tests/basic/main.tf)**
...
- Multiple volume types and attachment
  ...
  ebs_volumes = [
    {
      ...
      volume_size = 500
      volume_type = "io2"
      iops        = 10000    # Provisioned IOPS for database
      encrypted   = true
    },
    {
      device_name = "/dev/sdg"
      volume_size = 1000
      volume_type = "st1"    # Throughput-optimized for data warehouse
      encrypted   = true
    }
  ]
...
**IAM and bootstrap configuration:**
```hcl
module "app_server" {
  source = "../../"
...
- Spot pricing and interruption configuration
...
module "batch_processor" {
  ...
  # Large root volume for batch data
  root_block_device = {
    volume_size = 100
    volume_type = "gp3"
    encrypted   = true
...
**💰 Cost Comparison:** Spot instances can reduce costs by up to 90% compared to on-demand pricing. Ideal for fault-tolerant workloads like batch processing, CI/CD, and data analysis.
...
```
📁 [Complete example →](tests/with_ebs/main.tf)
...
```
📁 [Complete example →](tests/basic/main.tf)
...
**💡 For complete user data example with IAM roles and SSM integration:** [`tests/user_data/main.tf`](tests/user_data/main.tf)
...
**📁 Example:** [`tests/user_data/main.tf`](tests/user_data/main.tf)
...
**📁 Complete example:** [`tests/user_data/main.tf`](tests/user_data/main.tf)
...
**📁 Complete example:** [`tests/with_eip/main.tf`](tests/with_eip/main.tf)
