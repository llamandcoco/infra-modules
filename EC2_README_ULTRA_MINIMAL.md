...
- Flexible instance configuration (on-demand and spot instances)
- Storage options with EBS volumes and encryption
- Network configuration (VPC, security groups, Elastic IPs)
- IAM integration with automatic role creation
- Security best practices (IMDSv2, EBS encryption)
- User data support for instance bootstrap
...
|---------|-------------|-----------|
| Basic Instance | Minimal private EC2 with defaults | [`tests/basic/`](tests/basic/) |
| Web Server | Public instance with EIP, HTTP/HTTPS | [`tests/with_eip/`](tests/with_eip/) |
| Database Server | Multiple EBS volumes (io2, st1, gp3) | [`tests/with_ebs/`](tests/with_ebs/) |
| IAM & User Data | Bootstrap with IAM roles and SSM | [`tests/user_data/`](tests/user_data/) |
| Spot Instance | Cost-optimized batch processing | [`tests/spot_instance/`](tests/spot_instance/) |
...
- **AMI Selection**: Use AWS CLI to find latest AMIs ([see tests](tests/basic/main.tf#L31))
- **Instance Sizing**: [AWS Instance Types](https://aws.amazon.com/ec2/instance-types/)
- **Storage Types**: gp3 (default), io2 (high-performance), st1 (throughput) - [examples](tests/with_ebs/)
- **User Data**: Bash or cloud-init format - [example](tests/user_data/)
- **IAM Roles**: SSM, CloudWatch, S3 access patterns - [example](tests/user_data/)
- **Security Groups**: Web, app, bastion patterns - [example](tests/with_eip/)

## Security Best Practices

- **IMDSv2**: Required by default for metadata access
- **EBS Encryption**: Enabled by default for all volumes
- **Termination Protection**: Set `disable_api_termination = true` for production
