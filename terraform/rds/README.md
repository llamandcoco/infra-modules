...
| <a name="input_deletion_protection"></a> [deletion\_protection](#input\_deletion\_protection) | Whether to enable deletion protection. Prevents accidental deletion of the database. | `bool` | `false` | no |
...
| <a name="input_iam_database_authentication_enabled"></a> [iam\_database\_authentication\_enabled](#input\_iam\_database\_authentication\_enabled) | Whether to enable IAM database authentication. Allows authentication using AWS IAM credentials. | `bool` | `false` | no |
...
| <a name="input_master_username"></a> [master\_username](#input\_master\_username) | Username for the master DB user. Cannot be changed after creation. | `string` | n/a | yes |
| <a name="input_max_allocated_storage"></a> [max\_allocated\_storage](#input\_max\_allocated\_storage) | The upper limit of storage (GiB) to which RDS can automatically scale. Set to 0 to disable storage autoscaling. | `number` | `0` | no |
