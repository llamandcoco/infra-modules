...
# ECS Service Module
# Creates an ECS task definition, service, and auto-scaling configuration
# Requires an existing ECS cluster (created via ecs-cluster module)
# -----------------------------------------------------------------------------
...
locals {
  container_name = var.container_name != null ? var.container_name : var.service_name
}
...
resource "aws_appautoscaling_policy" "alb_request_count" {
  ...
  name               = "${var.service_name}-alb-request-count-scaling"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.this.resource_id
  scalable_dimension = aws_appautoscaling_target.this.scalable_dimension
  service_namespace  = aws_appautoscaling_target.this.service_namespace
